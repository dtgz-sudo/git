app.controller("goodsEditController", function ($scope, $http) {
    ...
    /**
     * $scope.$watch(要被监控的对象,被监控的对象变化执行的函数 )
     *方法： 监听entity 查看 输入框输入的数据是否正确
     */
    ...
    //$scope.$watch (需要监控的对象,监控对象变化的时候执行的方法)
    ...
    // 监控 三级分类 当三级分类选中的时候 后台去查询三级分类对应各 模板
    $scope.$watch("entity.tbGoods.typeTemplateId", function (newValue, oldValue) {
        ...
        $http.get('../typeTemplate/findOne/' + newValue).success(function (resp) {
            ...
            // 扩展属性
            $scope.entity.tbGoodsDesc.customAttributeItems = JSON.parse(resp.data.specIds);
        });
